...
export const toOriginColor = (color, inject) => {
    let prefix = ''
    if (inject) {
        prefix = `${inject}-${color} ${inject}-${color} `
    }
...
export const toOutlineColor = (color, inject) => {
    ...
    return prefix + `text-${color} border border-${color} bg-transparent`
}
...
const typeIntercept = (opts = {}) => {
    ...
    if (not('Null', presets) && not('Object', presets)) {
        throw new Error(`createColor 的配置 presets 期望是 Object 类型，但却得到了 ${showType(presets)} 类型`)
    }
}
...
export const createPresetsWithPrefix = (prefix) => {
    const presets = {}
    colors.forEach(color => presets[color] = `${prefix}-${color}`)
    return presets
}